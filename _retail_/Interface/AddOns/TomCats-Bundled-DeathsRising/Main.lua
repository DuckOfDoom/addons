...
function C_VignetteInfo.GetVignettes()
    ...
    for creatureID, creature in pairs(D["Creatures"].records) do
        ...
        if (vignetteInfo and vignetteInfo.name) then
            local location = D["Creatures"][creatureID]["Location"]
            if (location) then
...
if (HandyNotes) then
    ...
    local HandyNotesOptions = {
        ...
        get = function(info) return TomCats_Account.DeathsRisingEnableHandyNotesPlugin or false end,
        set = function(info, v)
